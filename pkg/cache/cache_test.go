...
func TestCacheWrap(t *testing.T) {
	c, _ := New(bucketsCount * chunkSize * 1.5)
	defer c.Reset()
	...
	if s.AllocBytes < 1024 {
		t.Fatalf("unexpected AllocBytes; got %d; cannot be smaller than %d", s.AllocBytes, 1024)
	}
	if s.MaxBytes < 32*1024*1024 {
		t.Fatalf("unexpected MaxBytes; got %d; cannot be smaller than %d", s.MaxBytes, 32*1024*1024)
	}
}
...
func TestCacheGetSetConcurrent(t *testing.T) {
	itemsCount := 10000
	const gorotines = 10
	c, _ := New(30 * itemsCount * gorotines)
	defer c.Reset()
